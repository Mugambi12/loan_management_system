...
import calendar
import os
from app.forms import RegistrationForm, LoginForm, EditUserForm, LoanForm, EditLoanForm, ContributionForm, EditContributionForm, RecordsForm, ReminderForm, SettingsForm
from app.models import db, User, Role, Verification, System, Contribution, Loan, Records, Reminder
...
def allowed_file(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in routes.config['ALLOWED_EXTENSIONS']

def allowed_docs_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in routes.config['ALLOWED_DOCS_EXTENSIONS']
    ...
    if form.validate_on_submit():
        ...
        if 'profile_picture' in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                filepath = os.path.join(routes.config['UPLOAD_FOLDER'], renamed_filename)
                file.save(filepath)
